...
extension TypedProgram {
  ...
  public func resolveReference(_ refString: String, in scope: AnyScopeID) -> Set<AnyDeclID>? {
      ...
      for declId in previousResult {
        let typeToLookIn =
          ...
          case ProductTypeDecl.self:
            ^ProductType(ProductTypeDecl.ID(declId)!, ast: ast)
          case TraitDecl.self:
            ^TraitType(TraitDecl.ID(declId)!, ast: ast)
          case TypeAliasDecl.self:
            // unwrap the metatype
            ((self[TypeAliasDecl.ID(declId)!].type).base as! MetatypeType).instance
          case NamespaceDecl.self:
            ^NamespaceType(NamespaceDecl.ID(declId)!, ast: ast)
          default:
