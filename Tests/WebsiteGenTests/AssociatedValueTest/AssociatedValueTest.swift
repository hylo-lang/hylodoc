...
final class AssociatedValueTest: XCTestCase {
  func test() throws {
    ...
    let associatedValueId = ast.resolveAssociatedValue(by: "B")!
    let targetId: AnyTargetID = .decl(AnyDeclID(associatedValueId))
    let doc = AssociatedValueDocumentation(
    ...
    let _ = documentation.symbols.associatedValueDocs.insert(doc, for: associatedValueId)

    var targetResolver: TargetResolver = .init()
    let partialResolved = partialResolveDecl(
      documentation, typedProgram, declId: AnyDeclID(associatedValueId))
